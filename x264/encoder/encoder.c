...
//      fprintf(stderr, "current: %p  prev: %p  oldest: %p \n", thread_current, thread_prev, thread_oldest);
    ...
    *pp_nal = NULL;
    ...
    if( pic_in != NULL )
    {
        /* 1: 读取一帧,存储在临时的fenc中. Copy the picture to a frame and move it to a buffer */
...
do_encode:
    ...
    /* X264_TYPE_IDR 一个GOP中的初始I帧 */
    if( h->fenc->i_type == X264_TYPE_IDR )
    {
        /* 重置所有参考帧, reset ref pictures */
    ...
    /* ------------------- Init                ----------------------------- */
    /* 初始化前向和后向参考帧, build ref list 0/1 */
    x264_reference_build_list( h, h->fdec->i_poc );

    /* 初始化帧级码流控制, Init the rate control */
    x264_ratecontrol_start( h, h->fenc->i_qpplus1 );
    /* 得到当前帧的QP(quantization parameter,量化参数) */
    ...
    /* ---------------------- Write the bitstream -------------------------- */
    /* 初始化比特流上下文, Init bitstream context */
    h->out.i_nal = 0;
    bs_init( &h->out.bs, h->out.p_bitstream, h->out.i_bitstream );
    ...
    if( i_nal_type == NAL_SLICE_IDR && h->param.b_repeat_headers )
    {
        ...
        /* 写序列参数集, generate sequence parameter set */
        /* 包括一个图像序列的所有信息，即两个IDR图像间的所有图像信息 */
        x264_nal_start( h, NAL_SPS, NAL_PRIORITY_HIGHEST );
        x264_sps_write( &h->out.bs, h->sps );
        x264_nal_end( h );

        /* 写画面参数集, generate picture parameter set */
    ...
    if( h->sh.i_type == SLICE_TYPE_P && !h->param.rc.b_stat_read 
        && h->param.i_scenecut_threshold >= 0 /* 场景切换检测门限(是否要插入额外的I帧) */
        && !h->param.b_pre_scenecut )
    {
