...
static int	  mk_flushContextData(mk_Context *c) {
  ...
  if (c->parent)
  {
    DEBUG_LOG(INF, "no fwrite %u bytes", c->d_cur);
    CHECK(mk_appendContextData(c->parent, c->data, c->d_cur));
  }
  else
  {
    DEBUG_LOG(INF, "fwrite %u bytes to %ld", c->d_cur, ftell(c->owner->fp));
    DEBUG_HEX(c->data, (c->d_cur > 16 ? 16 : c->d_cur));
...
int	  mk_close(mk_Writer *w) {
  ...
  if (mk_flushFrame(w) < 0 || mk_closeCluster(w) < 0)
    ret = -1;
  if (w->wrote_header) {
    DEBUG_LOG(INF, "w->duration_ptr = %d", w->duration_ptr);
