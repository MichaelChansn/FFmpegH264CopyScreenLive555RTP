...
#endif
#if (defined(_MSC_VER) || defined(SYS_SunOS) || defined(SYS_MACOSX)) && !defined(sqrtf)
#define sqrtf sqrt
#endif
#ifdef _WIN32
...
#pragma warning(disable:4293)//  add

static ALWAYS_INLINE intptr_t endian_fix( intptr_t x )
{
    if( WORD_SIZE == 8 )
        return endian_fix32(x>>32) + ((uint64_t)endian_fix32(x)<<32); /* #pragma warning(disable:4293) */
    else
