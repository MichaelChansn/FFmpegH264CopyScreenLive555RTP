...
int open_file_thread( char *psz_filename, hnd_t *p_handle, x264_param_t *p_param )
{
    ...
    h->frame_total = get_frame_total_yuv( h->p_handle );
...
int set_param_mkv( hnd_t handle, x264_param_t *p_param )
{
    ...
    /* i_sar_width,i_sar_height 设置的长宽比例 */
    if( p_param->vui.i_sar_width && p_param->vui.i_sar_height )
    {
...
int write_nalu_mkv( hnd_t handle, uint8_t *p_nalu, int i_size )
{
    ...
    // slice, sei
    case 0x1: // 一个非IDR图像的编码条带
    case 0x5: // IDR图像的编码条带
    case 0x6: // sei 补充的增强信息
    ...
    DEBUG_LOG(INF, "write_header_mkv: b_header_written=%d, pps=%p, sps=%p",
        p_mkv->b_header_written, p_mkv->pps, p_mkv->sps);
